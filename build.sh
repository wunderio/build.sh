...
#
#   build_sh_version_string: g
#       Build scripts version string.
#
build_sh_version_string = "build.sh 1.0"

#
#   Variable: build_sh_skip_backup
#       Trigger for skipping backups
#
build_sh_skip_backup = False

#
#   Variable: build_sh_disable_cache
#       Trigger for disabling caching
#
...
class MakeItem:
    ...
    #
    #   Function: validate
    #       Validate site.make item, returns a string describing the issue or
    #       False if no issues
    #
    #   Parameters:
    #       self - Description
    ...
    def validate(self):
        if 'type' in self.download_args:
            ...
            if self.download_args['type'] == 'git':
                if 'tag' not in self.download_args \
                        and 'revision' not in self.download_args:
                    return "No revision or tag defined for a git download"
            elif self.download_args['type'] == 'file' \
                    and 'url' in self.download_args \
                    and not version.match(self.download_args['url']):
                return "URL does not seem to have a version number in it ("
                + self.download_args['url'] + ")"
...
class Maker:
    ...
    def __init__(self, settings):
        ...
        if self.type == 'drush make':
            self.makefile = os.path.abspath(settings.get(
                'makefile',
                'conf/site.make'
            ))
            self.makefile_hash = hashlib.md5(
                open(self.makefile, 'rb').read()
            ).hexdigest()

    #
    #   Function: test
    #       Run tests on the makefile, print out the result
    #
    #   Parameters:
    #       self - Self
    #
    ...
    def install(self):
        if not self._drush([
            # "--root=" + self.final_build_dir + self.drupal_subpath,
            "site-install",
            self.profile_name,
    ...
    def _drush(self, args, quiet = False):
        # Add: See if drush is installed
        if quiet:
            FNULL = open(os.devnull, 'w')
            return subprocess.call([self.drush] + args, stdout=FNULL, stderr=FNULL, cwd=self.final_build_dir + self.drupal_subpath) == 0
        return subprocess.call([self.drush] + args, cwd=self.final_build_dir + self.drupal_subpath) == 0
